...
// NUMNODES is the number of nodes, or dots, subdividing the floor area
#ifndef NUMNODES
#define NUMNODES		4
#endif
...
int main( int argc, char *argv[ ] )
{
	...
	// {	
		...
		for( int i = 0; i < NUMNODES*NUMNODES; i++ )
		{
			...
			float edgeModifier = 1.0;	//modifer for edges

			if ((iu == 0 || iu == NUMNODES - 1) && (iv == 0 || iv == NUMNODES - 1))	//Corner
			{
				volume += (double)(edgeModifier * fullTileArea) * Height(iu, iv) / 4.0;
			}
			else if ((iu == 0 || iu == NUMNODES - 1) || (iv == 0 || iv == NUMNODES - 1)) //Edge
			{
		...
		// Need to double check the equation for this
